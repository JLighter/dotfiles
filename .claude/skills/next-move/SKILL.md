...
| **Tests are RED (ready to implement)** | Story has failing tests linked to AC |
| **Tests are GREEN (ready to refactor)** | Story tests pass but no refactor done yet |
...
7. Tests are RED → `/implement-feature US-NNN` to make them green.
8. Tests are GREEN → `/refactor` to improve the code, then `/review-feature`.
9. Stories ready (no tests yet) → `/generate-tests US-NNN` to start the Red phase.
10. Stories need refinement → `/new-feature` or refine the draft story.

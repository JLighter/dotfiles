...
Item {
  ...
  // Seule difference avec l'OSD d'origine : le volume ne s'affiche plus ici,
  // la barre local.menubar porte deja sa propre jauge. Le filtre ne vise que
  // les notifications de niveau (icone `volume-*` accompagnee d'un pourcentage) ;
  // le changement de peripherique de sortie, lui, n'envoie qu'un message et
  // continue de passer, comme la luminosite, le micro et le touchpad.
  function suppressedByBarGauge(iconName, rawValue) {
    return String(iconName || "").indexOf("volume") === 0 && String(rawValue || "") !== ""
  }

  function show(iconName, rawMessage, rawValue, rawMax, rawProgressText, rawDuration) {
    if (suppressedByBarGauge(iconName, rawValue)) return
